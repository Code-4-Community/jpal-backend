...
export class fixedSurveyTemplate1753738609912 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "survey_template" ADD COLUMN "greeting" text NOT NULL DEFAULT ' '`,
    );
    await queryRunner.query(
      `ALTER TABLE "survey_template" ADD COLUMN "closing" text NOT NULL DEFAULT ' ' `,
    );
    await queryRunner.query(
      `ALTER TABLE "paragraph" ADD COLUMN "surveyTemplateId" INTEGER NOT NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "paragraph" DROP CONSTRAINT "FK_paragraph_surveyTemplate"`,
    );
