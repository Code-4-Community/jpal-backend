...
export class addAssignmentStatus1640018111752 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "assignment" RENAME COLUMN "completed" TO "status"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "assignment" ADD "status" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `ALTER TABLE "assignment" RENAME COLUMN "status" TO "completed"`,
    );
  }
}
