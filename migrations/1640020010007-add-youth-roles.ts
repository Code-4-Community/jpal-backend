...
export class addYouthRoles1640020010007 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "youth" RENAME COLUMN "isControl" TO "role"`,
    );
    await queryRunner.query(`ALTER TABLE "youth" DROP COLUMN "role"`);
    await queryRunner.query(
      `CREATE TYPE "youth_role_enum" AS ENUM('treatment', 'control')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "youth" RENAME COLUMN "role" TO "isControl"`,
    );
  }
}
