...
export const amazonSESClientFactory: Provider<AWS.SES> = {
  ...
  useFactory: () => {
    assert(process.env.AWS_SES_ACCESS_KEY_ID !== undefined, 'AWS_SES_ACCESS_KEY_ID is not defined');
    assert(
      process.env.AWS_SES_SECRET_ACCESS_KEY !== undefined,
      'AWS_SES_SECRET_ACCESS_KEY is not defined',
    );
    assert(process.env.AWS_SES_REGION !== undefined, 'AWS_SES_REGION is not defined');
