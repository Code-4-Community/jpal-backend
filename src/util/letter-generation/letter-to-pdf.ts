...
function placeLetterBody(doc: jsPDF, letter: Letter, settings: TextSettings) {
  ...
  // Date
  const date = letter.date.toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  }); // e.g. November 1, 2017
...
function placeLetterFooter(doc: jsPDF, settings: TextSettings) {
  ...
  doc.rect(
    textStart,
    currentY - footerFillPadding,
    textEnd - textStart,
    footerLines.length * lineHeight + 1 * footerFillPadding,
    'F',
  );

  // ...then place text on background
  doc.text(footerLines, (textEnd + textStart - footerFillPadding) / 2, currentY, {
    align: 'center',
  });
...
export async function letterToPdf(
  letter: Letter,
  headerImageData: Uint8Array | null,
): Promise<Buffer> {
  ...
  if (headerImageData !== null) {
    ...
    const { width, height } = imageSize(headerImageData);
    doc.addImage(
      headerImageData,
      letterBodySettings.textStart - 0.25,
      1,
      width / pdfPPI,
      height / pdfPPI,
    );
