...
### Test coverage
Because NestJS makes testing relatively easy, and because tests greatly improve product quality, all projects that inherit this scaffold should have high test coverage. Projects which inherit this scaffold may enforce certain levels of test coverage as defined in `package.json` and measured by `npm run test:cov`. However, with extremely few exceptions, the following testing requirements should be met by all projects:

1. **All services and controllers in the project should be unit tested**. This means that all their dependencies are mocked and every *public* method is fully tested (i.e. every branch and statement in that method is tested).
...
One more note. When testing protected routes, like `GET /auth/me`, you'll find it impossible to authenticate (you'll always receive `403 Forbidden`). This is because the authentication middleware is applied in the root `AppModule`, not in `AuthModule`. The simplest solution is to manually apply it to the compiled server.
...
### Decorators
The @nestjs/swagger package already uses some of our controller decorators like `@Get()`, `@Body()`, and `@Param()`, for example. However, the NestJS decorators can't define everything. For example, they can't specify route descriptions, DTO properties, or response types. Thus, we'll have to do these on our own. 
